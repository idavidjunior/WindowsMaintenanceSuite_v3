...
function Get-RedundantPrograms {
    ...
    $byPublisher = @{}
    foreach ($item in $all) {
        $alreadyCaught = ($redundant | Where-Object { $_.ProductCode -eq $item.ProductCode }) -or
                         ($knownDups | Where-Object { $_.ProductCode -eq $item.ProductCode })
        if (-not $alreadyCaught -and $item.Publisher) {
            $pub = $item.Publisher -replace '(?i)\s+(Inc\.?|Corp\.?|Corporation|Ltd\.?|Limited|LLC|GmbH|S\.A\.|S\.A\.S|S\.r\.l\.|PLC|Pty|Ltda|Co\.|Company|Group|Holdings|Technologies|Software|Systems|International)', ''
            $pub = $pub.Trim()
            if ($pub -and $pub -notmatch '^Microsoft|^Google|^Adobe|^Apple|^Oracle|^Mozilla|^Amazon') {
                if (-not $byPublisher.ContainsKey($pub)) { $byPublisher[$pub] = @() }
                $byPublisher[$pub] += $item
            }
        }
    }
    foreach ($entry in $byPublisher.GetEnumerator()) {
        if ($entry.Value.Count -ge 2) {
            foreach ($item in $entry.Value) {
                $knownDups += [PSCustomObject]@{
                    GroupName = "$($entry.Key) (mesmo publisher)"
                    Name = $item.Name
                    Publisher = $item.Publisher
                    ProductCode = $item.ProductCode
                    UninstallString = $item.UninstallString
